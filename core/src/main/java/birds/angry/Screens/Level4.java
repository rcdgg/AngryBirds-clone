...
public class Level4 extends LevelScreen{
    ...
    private Body jupiter, moon, jp, mp;


    public Level4(Game game, String filepath) {
        super(game, filepath);

        world.setGravity(new Vector2(0, 0));

        //jupiter
        bodyDef.position.set(16*grid_size, 17f*grid_size);
        bodyDef.type = BodyDef.BodyType.StaticBody;
        jupiter = world.createBody(bodyDef);
        CircleShape cj = new CircleShape();
//        cj.setPosition(new Vector2(8*grid_size, 9*grid_size));
        cj.setRadius(8.5f*grid_size);
        fixtureDef.shape = cj;
        fixtureDef.isSensor = true;
        jupiter.createFixture(fixtureDef);

        //jupiter p
        bodyDef.position.set(14f*grid_size, 16.9f*grid_size);
        bodyDef.type = BodyDef.BodyType.StaticBody;
        jp = world.createBody(bodyDef);
        CircleShape cjp = new CircleShape();
        cjp.setRadius(3f*grid_size);
        fixtureDef.shape = cjp;
        fixtureDef.isSensor = true;
        jp.createFixture(fixtureDef);

        //moon
        bodyDef.position.set(25.5f*grid_size, 6.75f*grid_size);
        bodyDef.type = BodyDef.BodyType.StaticBody;
        moon = world.createBody(bodyDef);
        CircleShape cm = new CircleShape();
//        cj.setPosition(new Vector2(8*grid_size, 9*grid_size));
        cm.setRadius(6f*grid_size);
        fixtureDef.shape = cm;
        fixtureDef.isSensor = true;
